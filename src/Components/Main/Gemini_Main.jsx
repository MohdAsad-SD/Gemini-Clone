    ...
    <div onClick={()=>setopen(false)} className='bg-[#0e0e0f] flex h-full flex-1 p-6 overflow-hidden flex-col'>
      ...
      <div className='flex-1 flex flex-col mt-8  flex gap-6 flex-col  self-center items-center overflow-y-auto px-5 py-1 w-[100%]  md:w-[50%] overflow-x-hidden'>
        {message.map((msg,index)=>(
          <div key={index} className={`break-words flex rounded-2xl p-5 w-[80%] ${msg.type==="user" ? "":"text-start"} ${msg.type==="user" ? "bg-blue-500 self-end":"bg-[#2a2a2a] self-start"} ${msg.type==="user" ? "w-[auto]":"w[100%]"} ${msg.type==="ai" ? "text-white":"text-black"}`}>
            {msg.text}
          </div>
      ...
      <div className='w-full flex justify-center relative '>
        <textarea value={input} onChange={(e)=>setinput(e.target.value)} className='w-2xl resize-none bg-[#2a2a2a] outline-none rounded-2xl  text-white h-[100px] p-5' type="text" placeholder='Ask Gemini'></textarea>
